...
指针可以安全地指向栈对象，但要注意作用域结束后栈内存释放，指针变为悬挂指针。
...
int* p;    // 没有初始化，里面存的就是一个随机的垃圾值
*p = 10;   // 给一个随机位置的内容赋值，如果使用会造成严重后果  

int* p = (int*)0x12345678; // 随意赋个内存地址，如果使用会造成严重后果  
...
用来释放当前持有的资源，并可选地让智能指针指向新的资源。(对 weak 无效，weak 指针只能清空不能重指，weak 只有一个无参数版本的 reset())
