...
Cheap SSS Light（廉价次表面散射光）就是一种“假装有次表面散射”的光照技巧，专门用来做：皮肤、耳朵、叶子那种“透光发亮”的感觉

但不做真正复杂的 SSS 计算（因为太贵）

让背光区域泛出一圈柔和的亮光，模拟光穿过物体

真实的 SSS（Subsurface Scattering）是：光进入物体内部，在里面散射，再从别的地方出来。很贵（计算复杂），所以游戏里常用“cheap fake”。
