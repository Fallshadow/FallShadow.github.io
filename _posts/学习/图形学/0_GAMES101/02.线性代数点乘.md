...
\[
I_s = k_s\, I_L\, \max(0,\mathbf{N}\cdot \mathbf{H})^{s}
\]

其中：
- $\mathbf{N}$ 是指向视点/相机的单位向量
- $\mathbf{H}$ 是半程向量（half-vector）
- $k_s$ 为镜面反射系数
- $s$ 为高光指数
...
\[
\mathbf a_{\parallel}=\mathrm{proj}_{\mathbf b}(\mathbf a)=\left(\{11}{5},\{22}{5}\right)
\]
